...
**List envelopes and proof:** **`GET .../scans/{id}/executions`** and **`GET .../scans/{id}/findings`** each return one shared **`scan_navigation`** object per response (not per row): path-only **`findings_list_path`**, **`executions_list_path`**, and **`run_status_path`** for that scan. Those three strings must match **`GET .../run/status`** **`drilldown`** so clients can pivot without resynthesizing URLs. **CI** exercises this via **`TestContract_executionList_wireKeys`** and **`TestContract_findingsList_wireKeys`** in **`internal/api/read_contract_test.go`** (envelope keys + **`scan_navigation`** equals **`NewScanListNavigation`** for the request scan id). **Local Docker** **`make e2e-local`** / **`make benchmark-findings-local`** additionally assert **`scan_navigation`** against live **`drilldown`** (see **`scripts/read_trust_assert.sh`** and [testing.md](testing.md#ci-vs-local)).
