...
| **Findings** (list / GET by id / evidence) | Assessment output; list is triage-only; detail includes **`evidence_summary`**; evidence route returns artifact bodies. The persisted **`summary`** string on each finding mirrors the operator one-liner from the mutation runner: when **`assessment_tier`** is not **`confirmed`** and assessment notes exist (for example **`weak_matcher_signal`**), **`summary`** appends **`; assessment: <comma-joined note codes>`** so operators see why the row is not confirmed without fetching detail. | From scan or endpoint-filtered finding list; follow **`evidence_uri`** and ids from list rows to detail. |
