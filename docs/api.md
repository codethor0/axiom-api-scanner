...
**List envelopes and proof:** **`GET .../scans/{id}/executions`** and **`GET .../scans/{id}/findings`** each return one shared **`scan_navigation`** object per response (not per row): path-only **`findings_list_path`**, **`executions_list_path`**, and **`run_status_path`** for that scan. Those three strings must match **`GET .../run/status`** **`drilldown`** so clients can pivot without resynthesizing URLs. **CI** exercises this via **`TestContract_executionList_wireKeys`**, **`TestContract_findingsList_wireKeys`**, and **`TestContract_scanRunStatus_wireKeys_withCoverageAndDiagnostics`** in **`internal/api/read_contract_test.go`** (list envelopes + **`drilldown`** list paths all align with **`NewScanListNavigation`** for the scan id). **Local Docker** **`make e2e-local`** / **`make benchmark-findings-local`** additionally assert **`scan_navigation`** against live **`drilldown`** over **`curl`** (see **`scripts/read_trust_assert.sh`** and [testing.md](testing.md#proof-matrix-ci-vs-local-vs-environment)).
...
Optional **filter** query parameters (all exact match, ANDed with pagination): `assessment_tier` (**`confirmed`**, **`tentative`**, or **`incomplete`**), **`severity`** (**`info`**, **`low`**, **`medium`**, **`high`**, **`critical`**), **`rule_declared_confidence`** (**`high`**, **`medium`**, **`low`**), **`rule_id`**, **`scan_endpoint_id`** (UUID of an imported endpoint row; limits findings to that operation). Invalid enum-like values or a bad UUID for **`scan_endpoint_id`** return `400` `invalid_filter`. As with executions, filtered pages return the same envelope shape, including one **`scan_navigation`** for the scan.
...
**`read_trust_legend` (always present on detail):** a fixed glossary object whose **property names align with the payload** (`severity`, `rule_declared_confidence`, `assessment_tier`, `evidence_summary`, `evidence_inspection`, `operator_assessment`, **`finding_list_row`**). Values are **stable strings** explaining how each axis or derived block relates to the others—**not** row-specific values and **not** a repeat of **`operator_assessment.evidence_sufficiency_guide`**. **`finding_list_row`** documents how the scan findings **list** projection differs from this detail. **`make benchmark-findings-local`** and **`make e2e-local`** assert non-empty strings for every legend key on real GETs (via **`scripts/read_trust_assert.sh`**).
