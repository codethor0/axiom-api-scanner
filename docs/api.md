...
| `findings_summary` | **Read-only** aggregates from **listing stored findings** for this scan: `total`, `by_assessment_tier`, `by_severity` (maps omit empty buckets). No new filters or endpoints. |
| `rule_family_coverage` | **Factual** signals for four stable V1 mutation families (whether **mutated** `execution_records` exist for rules of that family, joined with rules loaded from `AXIOM_RULES_DIR`). See **Rule family coverage** below. |
...
**Stable wire shape (contract):** Successful `200` responses include these top-level keys in order: `scan`, `run`, `progress`, `summary`, `findings_summary`, `rule_family_coverage`, `guidance`, `coverage`, `diagnostics`, `compatibility`. Nested **`guidance.next_steps`**, **`diagnostics.blocked_detail`**, **`diagnostics.skipped_detail`**, and **`diagnostics.consistency_detail`** are always arrays. Contract tests in `internal/api` assert presence for successful, blocked, failed, and coverage-unavailable scenarios.

**Read-model invariants (expected relationships, not enforced by repair):**

- When a **`FindingRepository` is configured**, `summary.findings_created` (from `scans.findings_count`) should equal `findings_summary.total` (from `ListByScanID`). If not, `diagnostics.consistency_detail` includes `findings_count_drift` citing both integers.
...
**Rule family coverage:** JSON keys: `idor_path_or_query_swap` (`replace_path_param` / `replace_query_param`), `mass_assignment_privilege_injection` (`merge_json_fields`), `path_normalization_bypass` (`path_normalization_variant`), `rate_limit_header_rotation` (`rotate_request_headers`). Each entry includes `exercised` (at least one matching mutated execution), `rules_in_pack` (count of loaded rules whose YAML mutations include that family), `mutated_executions` (row count of mutated executions for rules of that family), and optional `not_exercised_reason` (`code` + `detail`) when `exercised` is false. Reasons are **only** one of: no rules for that family in the pack; mutation pass not succeeded yet; `mutation_candidates_total` is 0 on the scan row; or no mutated rows for rules of that family while the mutation pass succeeded with non-zero candidates. Top-level `unavailable` is set when the rules directory is not configured on the API instance, rules failed to load, or the execution repository is missing (per-entry mirrors in those cases).
