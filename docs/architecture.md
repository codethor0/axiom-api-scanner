...
6. `GET /v1/scans/{id}/endpoints` returns paginated inventory rows (optional per-row **`summary`** counts); **`GET .../endpoints/{endpointID}`** returns one row with **always-on summary** and **`drilldown.scan_endpoint_id`** so operators can filter executions and findings lists without embedding related objects in the endpoint payload.
