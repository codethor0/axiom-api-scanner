...
Finding rows store **severity** (impact), **rule_declared_confidence** (YAML `high`/`medium`/`low`), and **assessment_tier** (`confirmed`/`tentative`/`incomplete`) as separate columns—no field overloads. Assessment uses rule severity, declared confidence, weak matcher signals, and HTTP evidence completeness. `evidence_summary` JSON may duplicate tier and declared confidence for artifact bundles; **`GET /v1/findings/{id}`** still treats the row + normalized read model as primary, merges baseline/mutated execution IDs from the summary JSON when columns are empty, trims confidence/tier strings for display, and exposes **`evidence_inspection.matcher_outcomes`** sorted by matcher index for stable inspection. Findings are inserted only from the mutation path when diff evaluation passes and is not incomplete; **`evidence_artifacts`** hold raw request/body snapshots and a short diff line (distinct from **`execution_records`** full exchange GETs).
