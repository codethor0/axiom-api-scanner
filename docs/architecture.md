...
6. `GET /v1/scans/{id}/executions` returns paginated **execution list** rows (summaries only); `GET .../executions/{executionID}` returns the full redacted exchange (optional narrow filters on the list; validated query parameters).
