...
HTTP API lists executions and scan findings behind a **stable list envelope**: `items` plus `meta` (`limit`, `sort`, `order`, `has_more`, `next_cursor`). Pagination is **keyset** (opaque `cursor`); default limit 50, max 200; deterministic sort with `id` tie-break. Execution list sorts: `created_at`, `phase`. Finding list sorts: `created_at`, `severity`. Nested **execution read** rows include redacted `request` / `response`, `execution_kind`, `candidate_key`, `mutation_rule_id`, and **`request_summary` / `response_summary`**. **Finding read** rows match persisted columns plus optional **`evidence_inspection`**. **`GET .../run/status`** builds `findings_summary` via `FindingRepository.SummarizeFindingsForScan` and protected-route / rule-family coverage via `ExecutionRepository.ListExecutionRunTallies` (minimal columns), avoiding loading full findings or execution bodies for that read path. Paginated list routes are unchanged.
