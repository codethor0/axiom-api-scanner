...
HTTP API lists executions and scan findings behind a **stable list envelope**: `items` plus `meta` (`limit`, `sort`, `order`, `has_more`, `next_cursor`). Pagination is **keyset** (opaque `cursor`); default limit 50, max 200; deterministic sort with `id` tie-break. Execution list sorts: `created_at`, `phase`. Finding list sorts: `created_at`, `severity`. **`GET .../executions`** returns **list items**: phase, rule linkage, **`request_summary` / `response_summary`**, timing—no full `request` / `response` snapshots (those appear only on **`GET .../executions/{id}`**). **`GET .../findings`** returns **list items**: triage-ranked fields, linkage, **`evidence_inspection`**, no raw **`evidence_summary`** blob (full **`FindingRead`** on **`GET /v1/findings/{id}`**). **`GET .../run/status`** builds `findings_summary` via `FindingRepository.SummarizeFindingsForScan` (**one aggregate query** on PostgreSQL; no full finding rows) and loads scan endpoints via **`ListScanEndpointsForRunStatus`** (planner + declared-security fields only, not content-type metadata). Protected-route and rule-family coverage still use **`ExecutionRepository.ListExecutionRunTallies`** (minimal columns), not full execution bodies. Per-endpoint inventory counts on **`GET .../endpoints`** reuse a shared SQL CTE fragment for baseline/mutated execution and finding row counts.
