...
HTTP API lists executions and scan findings behind a **stable list envelope**: `items` plus `meta` (`limit`, `sort`, `order`, `has_more`, `next_cursor`). Pagination is **keyset** (opaque `cursor`); default limit 50, max 200; deterministic sort with `id` tie-break. Execution list sorts: `created_at`, `phase`. Finding list sorts: `created_at`, `severity`. Nested **execution read** rows include redacted `request` / `response`, `execution_kind`, `candidate_key`, `mutation_rule_id`, and **`request_summary` / `response_summary`**. **Finding read** rows match persisted columns plus optional **`evidence_inspection`**. Scan-run status and other internals still use unbounded repository `ListExecutions` / `ListByScanID` for aggregates, not the paginated HTTP list API.
