...
- **`evidence_summary`** (JSON, `schema_version` **1**): `rule_id`, baseline and mutated execution ids, endpoint method/path template, `matcher_outcomes` (index, kind, pass, short `summary`), `diff_points`, **`assessment_tier`** (post-run sufficiency snapshot, same as the column), **`rule_severity`** and **`impact_severity`** (same impact bucket as YAML `severity` / finding `severity`—both keys are written for readability; legacy rows may have only `rule_severity`), **`rule_declared_confidence`** (same as column), optional `assessment_notes`, optional **`interpretation_hints`** (stable scanner-policy strings that gloss **why the tier is not confirmed**—orthogonal to `assessment_notes`, which record weak-signal causes). **`bench_*`** harness codes from the local quality benchmark are **not** stored here. **`assessment_tier`** is **not** a substitute for **`confidence`**; **`rule_severity` / `impact_severity`** are **not** confidence or tier.
