...
- **`summary`** (column, list + detail): same base sentence as today (`rule … matched for …`); if tier is not **`confirmed`** and **`assessment_notes`** is non-empty, the runner appends **`; assessment: <codes>`** (comma-joined) so weak-signal and similar rows are legible in **`GET .../findings`** without opening **`evidence_summary`**.
