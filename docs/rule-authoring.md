...
- **`summary`** (column, list + detail): same base sentence as today (`rule … matched for …`); if tier is not **`confirmed`** and **`assessment_notes`** is non-empty, the runner appends **`; assessment: <codes>`** (comma-joined, same order as persisted notes) so triage matches **`evidence_summary`** without re-parsing JSON.
