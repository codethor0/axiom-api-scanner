...
| Handler | Scan create, control, OpenAPI, PATCH scan, executions list/detail, baseline/mutations contract cases, scan run status (canonical vs `compatibility`, failure field semantics, diagnostics codes, `summary` / `findings_summary` / `rule_family_coverage` / `guidance`, cancel) and mem dedupe assertions using `httptest` and in-memory repository fakes (no database). |
