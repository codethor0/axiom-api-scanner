...
| Handler | Scan create, control, OpenAPI, PATCH scan, executions list/detail, baseline/mutations contract cases, scan run status (canonical vs `compatibility`, failure field semantics, diagnostics codes, `summary` / `findings_summary` / `rule_family_coverage` / `guidance`, `diagnostics.consistency_detail`, run/status **wire-shape** tests for successful/blocked/failed/coverage-unavailable responses, findings-count drift, cancel) and mem dedupe assertions using `httptest` and in-memory repository fakes (no database). |
