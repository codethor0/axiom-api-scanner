...
| Handler | Scan create, control, OpenAPI, PATCH scan, executions list/detail (**list envelope** `items`/`meta`, keyset `cursor`, sort `created_at`/`phase`, reject `offset` and bad cursor), baseline/mutations contract cases, **finding list** (same envelope, sort `severity`/`created_at`), **finding detail** (`FindingRead`, `evidence_inspection`, `rule_id` filter), **read contract** tests (execution list/detail, finding detail + evidence artifact keys, scan run status with `coverage` / `protected_route_coverage` / `diagnostics` keys), scan run status (canonical vs `compatibility`, failure field semantics, diagnostics codes, `summary` / `findings_summary` / `rule_family_coverage` / `guidance` / **`protected_route_coverage`**, `diagnostics.consistency_detail`, run/status **wire-shape** tests including public-only, blocked auth, execution-repo-unavailable, successful/blocked/failed paths, findings-count drift, cancel) and mem dedupe assertions using `httptest` and in-memory repository fakes (no database). **Aggregate read path:** `SummarizeFindingsForScan` / `ListExecutionRunTallies` in-memory parity tests (`scanrun_readmodel_test`); postgres `TestFindingWrite_integration` asserts aggregates match full list/tally projections when `AXIOM_TEST_DATABASE_URL` is set. |
| Baseline | `internal/executor/baseline/runner_test` uses `httptest` plus in-memory store; performs one GET baseline. |
| Integration | `internal/storage/postgres` when `AXIOM_TEST_DATABASE_URL` is set (runs `dbmigrate.Up` from `AXIOM_TEST_MIGRATIONS_DIR` or repo-root `migrations/`, through `000007_scan_run_orchestration` and earlier). Includes scan-run read-model aggregate checks against full row loads in `TestFindingWrite_integration`. |
