...
| Handler | Scan create, control, OpenAPI, PATCH scan, **endpoint list** (`items` envelope, optional `summary` counts, query filters `method` / `declares_security` / `include_summary`, wire-shape contract tests in `endpoints_contract_test`), executions list/detail (**list envelope** `items`/`meta`, keyset `cursor`, sort `created_at`/`phase`, reject `offset` and bad cursor), baseline/mutations contract cases, **finding list** (same envelope, sort `severity`/`created_at`), **finding detail** (`FindingRead`, `evidence_inspection`, `rule_id` filter), **read contract** tests (execution list/detail, finding detail + evidence artifact keys, scan run status with `coverage` / `protected_route_coverage` / `diagnostics` keys), scan run status (canonical vs `compatibility`, failure field semantics, diagnostics codes, `summary` / `findings_summary` / `rule_family_coverage` / `guidance` / **`protected_route_coverage`**, `diagnostics.consistency_detail`, run/status **wire-shape** tests including public-only, blocked auth, execution-repo-unavailable, successful/blocked/failed paths, findings-count drift, cancel) and mem dedupe assertions using `httptest` and in-memory repository fakes (no database). **Aggregate read path:** `SummarizeFindingsForScan` / `ListExecutionRunTallies` in-memory parity tests (`scanrun_readmodel_test`); postgres `TestFindingWrite_integration` asserts aggregates and `ListEndpointInventory` summaries when `AXIOM_TEST_DATABASE_URL` is set. |
